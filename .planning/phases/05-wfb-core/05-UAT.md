...
result: blocked
blocked_by: root-permission
reason: "需要 Root 权限和 Monitor 模式才能测试 TUN 设备创建"

### 12. 实时调度优先级（需 Root）
expected: 服务端模式下，RX 线程优先级 99，调度器线程优先级 95，TX 线程优先级 90
result: blocked
blocked_by: root-permission
reason: "需要 Root 权限和 Monitor 模式才能验证实时调度优先级"
